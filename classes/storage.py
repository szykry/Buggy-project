...
class RolloutStorage(object):
    ...
    def _discount_rewards(self, final_value, discount=0.99):
        ...
        R = self._generate_buffer(self.num_envs).masked_scatter((1 - self.dones[-1]).byte(), final_value)   # warning
        ...
        for i in reversed(range(self.rollout_size)):
            ...
            # todo: add GAE
            R = self._generate_buffer(self.num_envs).masked_scatter((1 - self.dones[-1]).byte(),            # warning
